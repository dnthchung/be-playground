...
//       throw new HttpException("Too many request, try again later", HTTP_STATUS.TOO_MANY_REQUESTS, ErrorCode.AUTH_TOO_MANY_ATTEMPTS);
//     }
...
export class AuthService {
  ...
  public async forgotPassword(email: string) {
    ...
    if (count >= maxAttempts) {
      throw new HttpException("Too many requests, try again later", HTTP_STATUS.TOO_MANY_REQUESTS, ErrorCode.AUTH_TOO_MANY_ATTEMPTS);
    }
